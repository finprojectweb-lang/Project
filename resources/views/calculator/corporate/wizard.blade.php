...
.section-divider { margin: 32px 0 22px; display: flex; align-items: center; gap: 12px; }
.section-divider-line { flex: 1; height: 2px; background: #f1f5f9; border-radius: 2px; }
.section-divider-badge {
    display: inline-flex; align-items: center; gap: 8px;
    padding: 6px 16px; border-radius: 100px;
    background: #f0fdf4; border: 2px solid #bbf7d0;
    font-size: .78rem; font-weight: 800; color: #065f46;
    text-transform: uppercase; letter-spacing: .07em; white-space: nowrap;
}
.section-divider-badge .sd-num {
...
.damage-pillars { display: grid; grid-template-columns: repeat(3,1fr); gap: 18px; margin-bottom: 28px; }
...
.pillar-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 4px; background: #e2e8f0; }
...
.level-label { font-size: .72rem; font-weight: 700; color: #475569; text-transform: uppercase; letter-spacing: .06em; margin-bottom: 8px; display: block; }
...
.level-dot { width: 9px; height: 9px; border-radius: 50%; border: 2px solid #cbd5e1; flex-shrink: 0; transition: all .2s; }
...
.level-tag { font-size: .65rem; font-weight: 700; padding: 2px 7px; border-radius: 100px; flex-shrink: 0; white-space: nowrap; }
.tag-none   { background: #f1f5f9; color: #64748b; }
.tag-low    { background: #d1fae5; color: #065f46; }
.tag-medium { background: #fef3c7; color: #92400e; }
.tag-high   { background: #fee2e2; color: #991b1b; }
.damage-summary-box { background: #f0fdf4; border: 2px solid #bbf7d0; border-radius: 12px; padding: 16px 20px; }
.damage-summary-title { font-size: .78rem; font-weight: 700; color: #065f46; text-transform: uppercase; letter-spacing: .06em; margin-bottom: 10px; }
.damage-summary { display: flex; flex-wrap: wrap; gap: 8px; }
.dmg-pill { display: inline-flex; align-items: center; gap: 5px; padding: 5px 12px; border-radius: 100px; font-size: .78rem; font-weight: 600; border: 1.5px solid #e2e8f0; background: #f8fafc; color: #475569; transition: all .25s; }
...
.review-block h4 { font-size: .8rem; font-weight: 700; color: #10b981; text-transform: uppercase; letter-spacing: .08em; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 1px solid #e2e8f0; }
...
.rk { font-size: .8rem; color: #64748b; flex-shrink: 0; }
.rv { font-size: .82rem; color: #1e293b; font-weight: 500; text-align: right; }
.dmg-review-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; }
@media(max-width: 500px) { .dmg-review-grid { grid-template-columns: 1fr; } }
...
<div class="wizard-card">
    ...
    <div class="wizard-body">
        ...
        @if($errors->any())
        <div class="err-alert">
            ⚠️ <strong>Terjadi kesalahan, silakan periksa kembali:</strong>
        ...
        <div class="wizard-step active" data-step="1">
            ...
            <p class="step-description">Informasi identitas perusahaan dan data Person in Charge</p>

            <div class="section-divider">
                <div class="section-divider-badge"><span class="sd-num">A</span> Data Perusahaan</div>
            ...
            <div class="section-divider">
                <div class="section-divider-badge"><span class="sd-num">B</span> Data PIC</div>
                <div class="section-divider-line"></div>
            </div>
            <p style="color:#64748b;font-size:.88rem;margin-bottom:20px;margin-top:-8px;">Person in Charge yang bertanggung jawab atas perhitungan dan pembayaran kompensasi lingkungan.</p>
            <div class="form-row">
                <div class="form-group">
                    <label class="form-label">Nama PIC <span class="required">*</span></label>
                    <input type="text" name="pic_name" class="form-control" required placeholder="Nama lengkap PIC" value="{{ old('pic_name', Auth::check() ? Auth::user()->name : '') }}">
                </div>
                <div class="form-group">
                    <label class="form-label">Jabatan PIC <span class="required">*</span></label>
                    <input type="text" name="pic_position" class="form-control" required placeholder="Direktur Keuangan / HSE Manager / dll" value="{{ old('pic_position') }}">
                </div>
            </div>
            <div class="form-row">
                <div class="form-group">
                    <label class="form-label">Email PIC <span class="required">*</span></label>
                    <input type="email" name="pic_email" class="form-control" required placeholder="pic@perusahaan.com" value="{{ old('pic_email', Auth::check() ? Auth::user()->email : '') }}">
                </div>
                <div class="form-group">
                    <label class="form-label">No. Telepon PIC <span class="required">*</span></label>
                    <input type="tel" name="pic_phone" class="form-control" required placeholder="08xxxxxxxxxx" value="{{ old('pic_phone') }}">
                </div>
            </div>
        </div>
            ...
            <div class="damage-pillars">
                <div class="pillar-card land">
                    ...
                    <div class="level-options">
                        <label class="level-opt lvl-none selected" data-pillar="land" data-level="none"><input type="radio" name="damage[land]" value="none" checked><div class="level-dot"></div><div class="level-text"><div class="level-name">Tidak Ada</div><div class="level-desc">Tidak ada dampak terhadap tanah</div></div><span class="level-tag tag-none">—</span></label>
                        <label class="level-opt lvl-low" data-pillar="land" data-level="low"><input type="radio" name="damage[land]" value="low"><div class="level-dot"></div><div class="level-text"><div class="level-name">Ringan</div><div class="level-desc">Dampak terbatas, area &lt; 1 ha</div></div><span class="level-tag tag-low">Ringan</span></label>
                        <label class="level-opt lvl-medium" data-pillar="land" data-level="medium"><input type="radio" name="damage[land]" value="medium"><div class="level-dot"></div><div class="level-text"><div class="level-name">Sedang</div><div class="level-desc">Pencemaran nyata, 1–10 ha</div></div><span class="level-tag tag-medium">Sedang</span></label>
                        <label class="level-opt lvl-high" data-pillar="land" data-level="high"><input type="radio" name="damage[land]" value="high"><div class="level-dot"></div><div class="level-text"><div class="level-name">Berat</div><div class="level-desc">Deforestasi / degradasi &gt; 10 ha</div></div><span class="level-tag tag-high">Berat</span></label>
                    </div>
                </div>
                <div class="pillar-card air">
                    ...
                    <span class="level-label">Tingkat Dampak</span>
                    <div class="level-options">
                        <label class="level-opt lvl-none selected" data-pillar="air" data-level="none"><input type="radio" name="damage[air]" value="none" checked><div class="level-dot"></div><div class="level-text"><div class="level-name">Tidak Ada</div><div class="level-desc">Tidak ada emisi signifikan</div></div><span class="level-tag tag-none">—</span></label>
                        <label class="level-opt lvl-low" data-pillar="air" data-level="low"><input type="radio" name="damage[air]" value="low"><div class="level-dot"></div><div class="level-text"><div class="level-name">Ringan</div><div class="level-desc">Emisi di bawah ambang baku mutu</div></div><span class="level-tag tag-low">Ringan</span></label>
                        <label class="level-opt lvl-medium" data-pillar="air" data-level="medium"><input type="radio" name="damage[air]" value="medium"><div class="level-dot"></div><div class="level-text"><div class="level-name">Sedang</div><div class="level-desc">Melebihi baku mutu, dampak lokal</div></div><span class="level-tag tag-medium">Sedang</span></label>
                        <label class="level-opt lvl-high" data-pillar="air" data-level="high"><input type="radio" name="damage[air]" value="high"><div class="level-dot"></div><div class="level-text"><div class="level-name">Berat</div><div class="level-desc">Emisi masif, dampak regional</div></div><span class="level-tag tag-high">Berat</span></label>
                    </div>
                </div>
                <div class="pillar-card water">
                    ...
                    <span class="level-label">Tingkat Dampak</span>
                    <div class="level-options">
                        <label class="level-opt lvl-none selected" data-pillar="water" data-level="none"><input type="radio" name="damage[water]" value="none" checked><div class="level-dot"></div><div class="level-text"><div class="level-name">Tidak Ada</div><div class="level-desc">Tidak ada pencemaran air</div></div><span class="level-tag tag-none">—</span></label>
                        <label class="level-opt lvl-low" data-pillar="water" data-level="low"><input type="radio" name="damage[water]" value="low"><div class="level-dot"></div><div class="level-text"><div class="level-name">Ringan</div><div class="level-desc">Limbah cair memenuhi baku mutu</div></div><span class="level-tag tag-low">Ringan</span></label>
                        <label class="level-opt lvl-medium" data-pillar="water" data-level="medium"><input type="radio" name="damage[water]" value="medium"><div class="level-dot"></div><div class="level-text"><div class="level-name">Sedang</div><div class="level-desc">Melampaui baku mutu, badan air lokal</div></div><span class="level-tag tag-medium">Sedang</span></label>
                        <label class="level-opt lvl-high" data-pillar="water" data-level="high"><input type="radio" name="damage[water]" value="high"><div class="level-dot"></div><div class="level-text"><div class="level-name">Berat</div><div class="level-desc">Pencemaran sungai / laut skala besar</div></div><span class="level-tag tag-high">Berat</span></label>
                    </div>
                </div>
            </div>
        ...
        <div class="wizard-step" data-step="3">
            ...
            <div style="display:flex;gap:10px;align-items:flex-start;background:#dbeafe;border:2px solid #3b82f6;color:#1e40af;padding:14px 18px;border-radius:10px;margin-bottom:24px;font-size:.88rem;line-height:1.6;">
                🔒 <span>Data Anda diproses secara aman. Setelah dikirim, Anda akan diarahkan ke halaman <strong>hasil & pembayaran kompensasi</strong> untuk memilih skema dan program restorasi.</span>
            </div>
    ...
    <div class="wizard-footer">
        ...
        <div class="step-counter">Langkah <b id="stepNum">1</b> dari 3</div>
...
(function() {
    ...
    var lvlLabel = { none: '—', low: 'Ringan', medium: 'Sedang', high: 'Berat' };
    var pillCls  = { none: '', low: 'active-low', medium: 'active-medium', high: 'active-high' };
    var industryNames = {
        manufacturing: 'Manufaktur', chemical: 'Industri Kimia', mining: 'Pertambangan',
        cement: 'Semen & Material', pulp: 'Pulp & Kertas', palm_oil: 'Pabrik Kelapa Sawit',
        steel: 'Baja & Logam', construction: 'Konstruksi', transportation: 'Transportasi & Logistik',
        energy: 'Energi & Utilitas', agriculture: 'Pertanian & Perkebunan', service: 'Jasa & Layanan',
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.level-opt').forEach(function(opt) {
            opt.addEventListener('click', function() {
                var pillar = opt.dataset.pillar;
                var level  = opt.dataset.level;
                document.querySelectorAll('.level-opt[data-pillar="' + pillar + '"]').forEach(function(o) { o.classList.remove('selected'); });
                opt.classList.add('selected');
                opt.querySelector('input[type=radio]').checked = true;
                var pill  = document.getElementById('pill-' + pillar);
                var icons = { land: '🏔️', air: '💨', water: '💧' };
                var names = { land: 'Tanah', air: 'Udara', water: 'Air' };
                pill.className   = 'dmg-pill ' + pillCls[level];
    ...
    function updateUI() {
        ...
        document.getElementById('nextBtn').textContent = cur === total ? '🌿 Kirim & Lihat Hasil' : 'Selanjutnya →';
    }

    function validateStep(n) {
        var el = document.querySelector('.wizard-step[data-step="' + n + '"]');
        var fields = el.querySelectorAll('[required]');
        for (var i = 0; i < fields.length; i++) {
            if (!fields[i].value.trim()) {
                fields[i].focus();
                fields[i].style.borderColor = '#ef4444';
                (function(f) { setTimeout(function() { f.style.borderColor = ''; }, 2500); })(fields[i]);
                return false;
            }
        }
        return true;
    ...
    function makeBadge(lvl) {
        var map = { none: ['#f1f5f9','#64748b'], low: ['#d1fae5','#065f46'], medium: ['#fef3c7','#92400e'], high: ['#fee2e2','#991b1b'] };
        var col = map[lvl] || map.none;
        return '<span style="background:' + col[0] + ';color:' + col[1] + ';padding:3px 10px;border-radius:100px;font-size:.72rem;font-weight:700;">' + (lvlLabel[lvl] || '—') + '</span>';
    }

    function populateReview() {
        var fd   = new FormData(document.getElementById('calcForm'));
        var land  = getRadioVal('damage[land]');
        var air   = getRadioVal('damage[air]');
        var water = getRadioVal('damage[water]');
        var ind   = fd.get('industry_type') || '';
        var indDisplay = industryNames[ind] || ind || '—';
        var desc  = fd.get('damage_description') || '';

        var html =
            '<div class="review-grid">' +
                '<div class="review-block">' +
                    '<h4>🏭 Data Perusahaan</h4>' +
                    row('Nama', fd.get('company_name')) +
                    row('SIUP/NIB', fd.get('company_siup')) +
                    row('Email', fd.get('company_email')) +
                    row('Telepon', fd.get('company_phone') || '—') +
                    row('Kategori', indDisplay) +
                    row('Afiliasi', fd.get('company_affiliate') || 'Independen') +
                    row('Fasilitas', (fd.get('facility_count') || '—') + ' lokasi') +
                    row('Lokasi', fd.get('company_location')) +
                    row('Tahun', fd.get('calculation_year')) +
                '</div>' +
                '<div class="review-block">' +
                    '<h4>👤 Data PIC</h4>' +
                    row('Nama PIC', fd.get('pic_name')) +
                    row('Jabatan', fd.get('pic_position')) +
                    row('Email PIC', fd.get('pic_email')) +
                    row('Telepon PIC', fd.get('pic_phone')) +
                '</div>' +
                '<div class="review-block full">' +
                    '<h4>🌍 Dampak Lingkungan</h4>' +
                    '<div class="dmg-review-grid">' +
                        '<div class="review-row"><span class="rk">🏔️ Tanah</span><span class="rv">' + makeBadge(land) + '</span></div>' +
                        '<div class="review-row"><span class="rk">💨 Udara</span><span class="rv">' + makeBadge(air) + '</span></div>' +
                        '<div class="review-row"><span class="rk">💧 Air</span><span class="rv">' + makeBadge(water) + '</span></div>' +
                    '</div>' +
                    (desc ? '<div class="review-row" style="margin-top:10px;"><span class="rk">Keterangan</span><span class="rv">' + desc + '</span></div>' : '') +
                '</div>' +
            '</div>';
    ...
    function row(label, val) {
        return '<div class="review-row"><span class="rk">' + label + '</span><span class="rv">' + (val || '—') + '</span></div>';
    }
})();
</script>
